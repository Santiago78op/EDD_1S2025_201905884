...
public class MerkleTree : IMerkleTree, IDisposable
{
    ...
    /// <summary>
    /// Genera una representación visual del árbol de Merkle en formato DOT para visualización con Graphviz.
    /// </summary>
    /// <returns>String en formato DOT</returns>
    public string GenerarDotFacturas()
    {
        ...
        dot.AppendLine("    label=\"Árbol Merkle de Comprobantes\";");
        ...
        else
        {
            ...
            int idCounter = 0;
            GenerarDotRecursivo(Root, dot, nodeIds, ref idCounter);
        }
    ...
    /// <summary>
    /// Método recursivo para generar la representación DOT de un nodo y sus hijos.
    /// </summary>
    private void GenerarDotRecursivo(NodeMerkleTree node, StringBuilder dot, Dictionary<string, int> nodeIds, ref int idCounter)
    {
        if (node == null) return;

        // Asignar un ID único al nodo si aún no tiene uno
        ...
        // Generar la etiqueta del nodo según si es un nodo hoja o interno
        string label;
        Factura factura = node.Value as Factura;
        if (factura != null) // Es un nodo hoja
        {
            label = $"\"Factura {factura.Id}\\nId Servicio: {factura.IdServicio}\\nTotal: {factura.Total}" +
                    $"\\nFecha: {factura.FechaCreacion}\\nMétodo Pago: {factura.MetodoPago}" +
        ...
        dot.AppendLine($"  node{nodeId} [label={label}, fillcolor=lightgreen];");
        ...
        if (node.Left != null)
        {
            ...
            dot.AppendLine($"  node{nodeId} -> node{leftId} [color=blue, label=\"L\"];");
            GenerarDotRecursivo(node.Left, dot, nodeIds, ref idCounter);
        }
        ...
        if (node.Right != null)
        {
            ...
            dot.AppendLine($"  node{nodeId} -> node{rightId} [color=green, label=\"R\"];");
            GenerarDotRecursivo(node.Right, dot, nodeIds, ref idCounter);
        }
    }
